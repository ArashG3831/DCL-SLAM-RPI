...
wait_lifecycle_active()
{
  ...
  while true; do
    ...
    if (( elapsed_s - last_print >= 10 )); then
      echo "[batch] still waiting for $node (${elapsed_s}s): ${state:-<no response>}"
      last_print="$elapsed_s"
    fi
...
start_stack()
{
  ...
  echo "[batch] automatically detecting D500 lidar..."
  ...
  if [ -n "${D500_PORT:-}" ]; then
    ...
    echo "[batch] Available persistent serial devices:"
    ls -l /dev/serial/by-id/ 2>/dev/null || true
    echo "[batch] Available USB serial devices:"
    ls -l /dev/ttyUSB* /dev/ttyACM* 2>/dev/null || true
    echo "[batch] USB devices:"
    lsusb || true
    return 1
  fi
  ...
  echo "[batch] checking raw lidar serial bytes..."
  stty -F "$PORT" 230400 raw -echo >/dev/null 2>&1 || {
    echo "[batch] ERROR: could not configure lidar serial port"
    return 1
  }

  raw_bytes="$(
    timeout 1 dd if="$PORT" bs=1 count=128 status=none 2>/dev/null |
      wc -c
  )"

  echo "[batch] raw lidar bytes received: $raw_bytes"

  if [ "${raw_bytes:-0}" -eq 0 ]; then
    echo "[batch] ERROR: no raw lidar serial data received"
    return 1
  fi

  sleep 1

  echo "[batch] starting motor node..."
  ...
  local motor_pid=$!
  printf '%s\n' "$motor_pid" > "$run_dir/motor_node.pid"

  sleep 2

  if ! kill -0 "$motor_pid" 2>/dev/null; then
    echo "[batch] ERROR: motor node exited during startup"
    tail -120 "$log_dir/01_motor.log" 2>/dev/null || true
    return 1
  fi

  echo "[batch] starting D500 lidar with automatic recovery..."
  ...
  echo "[batch] starting lidar static TF..."
  ...
  echo "[batch] starting SLAM Toolbox..."
  ...
  # Do not trust topic names alone. Require live data.
  echo "[batch] waiting for a real message on /scan ..."

  local scan_ok=0
  local scan_attempt

  for scan_attempt in 1 2 3; do
    if timeout 10 ros2 topic echo --once \
    ...
    echo "[batch] /scan check attempt $scan_attempt/3 failed; retrying..."
    sleep 1
  done

  if [ "$scan_ok" -eq 1 ]; then
    echo "[batch] received a message on /scan"
  else
    echo "[batch] ERROR: no /scan message received after 3 explicit checks"
    echo "[batch] Lidar diagnostic tail:"
    tail -80 "$log_dir/02_lidar.log" 2>/dev/null || true
    echo "[batch] live /scan endpoint details:"
    timeout 5 ros2 topic info -v /scan 2>&1 || true
    return 1
  fi

  wait_for_message /odom 30 || {
    echo "[batch] Motor/odom diagnostic tail:"
    tail -80 "$log_dir/01_motor.log" 2>/dev/null || true
    return 1
  }

  wait_for_message /map 60 --qos-durability transient_local --qos-reliability reliable || {
    echo "[batch] SLAM diagnostic tail:"
    tail -120 "$log_dir/04_slam.log" 2>/dev/null || true
    return 1
  }

  # Nav2 global costmap cannot activate until this complete TF exists.
  wait_for_tf map base_link 60 || {
    echo "[batch] SLAM diagnostic tail:"
    tail -120 "$log_dir/04_slam.log" 2>/dev/null || true

    echo "[batch] Current frames/topics diagnostic:"
    timeout 8 ros2 topic echo --once /tf 2>/dev/null | head -100 || true
    return 1
  }

  echo "[batch] SLAM map and map->base_link TF are live."
  sleep 2

  echo "[batch] starting Nav2..."
  ros2 launch my_epuck_project real_nav2_live_slam_launch.py \
    > "$log_dir/05_nav2.log" 2>&1 &
  # Lifecycle manager activates controller, planner, behavior and BT
  # in order. bt_navigator active therefore means the complete Nav2 stack
  # finished activation.
  wait_lifecycle_active /bt_navigator 60 || return 1
  echo "[batch] complete Nav2 stack is active"
  echo "[batch] clearing Nav2 costmaps..."
  timeout 10 ros2 service call /global_costmap/clear_entirely_global_costmap nav2_msgs/srv/ClearEntireCostmap {} \
    > "$log_dir/06_clear_global_costmap.log" 2>&1 || true
  timeout 10 ros2 service call /local_costmap/clear_entirely_local_costmap nav2_msgs/srv/ClearEntireCostmap {} \
    > "$log_dir/07_clear_local_costmap.log" 2>&1 || true

  sleep 2

  echo "[batch] starting gold diagnostic logger..."
  mkdir -p "$run_dir/gold"
  ...
  echo "[batch] starting frontier exploration..."
